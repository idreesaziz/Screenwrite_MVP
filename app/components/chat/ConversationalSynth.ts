...
export class ConversationalSynth {
  ...
  constructor(apiKey?: string, getToken?: () => Promise<string | null>) {
    // API key is deprecated - backend handles AI calls now
    // getToken is used for authenticated API calls
...
${conversationHistory}`;
    ...
    try {
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:streamGenerateContent?key=${GEMINI_API_KEY}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
